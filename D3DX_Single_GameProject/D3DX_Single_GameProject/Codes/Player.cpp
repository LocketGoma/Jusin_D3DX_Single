...
_int CPlayer::LateUpdate_GameObject(const _float& fDeltaTime)
{
	...
	m_pManagement->Add_RenderList(Engine::RENDERID::RENDER_UI, this);
	...
	if (m_iShieldHP == 45 && m_bLowShield == false)
	{
		...
		m_pManagement->Play_Sound(L"hev_45p.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
	}
	...
	if (m_bLowHP == true && !m_pManagement->Check_PlaySound(Engine::SOUND_CHANNELID::PLAYERSUIT))
	{
		m_pManagement->Stop_Sound(Engine::SOUND_CHANNELID::PLAYERSUIT);
		switch(rand() % 4)
		{
			case 0:
				m_pManagement->Play_Sound(L"hev_hlth2.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
			case 1:
				m_pManagement->Play_Sound(L"hev_hlth3.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
			case 2:
				m_pManagement->Play_Sound(L"hev_hlth5.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
			case 3:
				m_pManagement->Play_Sound(L"hev_hlth6.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
		}
		m_bLowHP = false;
	}
...
_bool CPlayer::HurtState()
{
	...
	if (m_ibHP > m_iHP || m_ibShieldHP > m_iShieldHP)
	{
		if (m_iShieldHP == 0)
		{
			...
			{
			case 0:
				m_pManagement->Play_Sound(L"hev_dmg2.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
			case 1:
				m_pManagement->Play_Sound(L"hev_dmg4.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
			case 2:
				m_pManagement->Play_Sound(L"hev_dmg6.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
				break;
			}
		...
		{
		case 0:
			m_pManagement->Play_Sound(L"pl_fallpain1.wav", Engine::SOUND_CHANNELID::PLAYER);
			break;
		case 1:
			m_pManagement->Play_Sound(L"pl_pain5.wav", Engine::SOUND_CHANNELID::PLAYER);
			break;
		case 2:
			m_pManagement->Play_Sound(L"pl_pain6.wav", Engine::SOUND_CHANNELID::PLAYER);
			break;
		case 3:
...
HRESULT CPlayer::Add_Component(void)
{
	...
	// Transform
	pComponent = m_pTransformCom = dynamic_cast<Engine::CTransform*>(m_pManagement->Clone_Prototype(L"Transform_Comp"));
	...
	m_pStatusUI = dynamic_cast<CStatusUI*>(m_pManagement->Clone_GameObject(L"StatusUI"));
	
	m_pCrossHairUI = dynamic_cast<CCrossHairUI*>(m_pManagement->Clone_GameObject(L"CrossHairUI"));
	
	m_pWeaponUI = dynamic_cast<CWeaponUI*>(m_pManagement->Clone_GameObject(L"WeaponUI"));


	//아래는 위치 옮겨둘것

	m_pWeapon[(_uint)eWeaponType::WEAPON_CROWBAR] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponCrowbar"));

	m_pWeapon[(_uint)eWeaponType::WEAPON_PISTOL] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponPistol"));
	
	m_pWeapon[(_uint)eWeaponType::WEAPON_SMG] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponSMG"));

	m_pWeapon[(_uint)eWeaponType::WEAPON_RIFLE] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponRifle"));

	m_pWeapon[(_uint)eWeaponType::WEAPON_SHOTGUN] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponShotgun"));

	m_pWeapon[(_uint)eWeaponType::WEAPON_PHYCANNON] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponPhysCannon"));
	
	//m_pWeapon[(_uint)eWeaponType::WEAPON_RPG] = dynamic_cast<CPlayerWeapon*>(m_pManagement->Clone_GameObject(L"WeaponRPG"));
...
{
	Engine::CManagement* m_pManagement = Engine::CManagement::Get_Instance();
	...
	m_pManagement->Render_Font(L"Font_BASE", m_szPos, &_vec2(0,60), D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f));
...
void CPlayer::Key_Input(const _float& fDeltaTime)
{
	...
	if (m_pManagement->Key_Pressing('W'))
	{
 		m_pTransformCom->Move_Pos(&vLook, m_fNowMoveSpeed, fDeltaTime);
	}
	if (m_pManagement->Key_Pressing('S'))
	{
		m_pTransformCom->Move_Pos(&vLook, m_fNowMoveSpeed, -fDeltaTime);
	}
	if (m_pManagement->Key_Pressing('A'))
	{
		m_pTransformCom->Move_Pos(&vRight, m_fNowMoveSpeed, -fDeltaTime);
	}
	if (m_pManagement->Key_Pressing('D'))
	{
		m_pTransformCom->Move_Pos(&vRight, m_fNowMoveSpeed, fDeltaTime);
	}
	if (m_pManagement->Key_Pressing(VK_LSHIFT))
	{
		if (m_bSprint == false)
		{
			m_pManagement->Stop_Sound(Engine::SOUND_CHANNELID::PLAYERSUIT);
			m_pManagement->Play_Sound(L"suit_sprint.wav", Engine::SOUND_CHANNELID::PLAYERSUIT);
	...
	if (m_pManagement->Key_Pressing(VK_SPACE))
	{
		m_bJump = true;
	}
	...
	//사격
	m_bShootState = false;
	if (m_pManagement->Key_Pressing(VK_LBUTTON))
	{
		m_bShootState = m_pWeapon[(_uint)m_pWeaponType]->Shoot_Weapon();		
	}	
	if (m_pManagement->Key_Down(VK_RBUTTON))
	{
		m_pWeapon[(_uint)m_pWeaponType]->AltShoot_Weapon();
		m_bShootState = true;
	}
	else if (m_pManagement->Key_Pressing(VK_RBUTTON) && m_pWeaponType == eWeaponType::WEAPON_PHYCANNON)
	{
		if (dynamic_cast<CWeaponPhysCannon*>(m_pWeapon[(_uint)m_pWeaponType])->Grap_ItemState() == false && dynamic_cast<CWeaponPhysCannon*>(m_pWeapon[(_uint)m_pWeaponType])->Get_LookTargetObject()!=nullptr)
			m_pWeapon[(_uint)m_pWeaponType]->AltShoot_Weapon();
	}

	if (!m_pManagement->Key_Pressing(VK_LBUTTON) && m_pWeapon[(_uint)m_pWeaponType]->Is_End_Animation())
	{
	...
	else if (g_zDelta < -0.1f)
	{
		...
		if (m_pWeaponType == eWeaponType::WEAPON_CROWBAR)
		{
			...
			m_pWeaponType = eWeaponType::WEAPON_PHYCANNON;		//최종적으로는 마지막무기
		}
		else
		{
			m_pWeaponType = (eWeaponType)((_uint)m_pWeaponType - 1);
